...
import { UserCheck, Search, UserPlus, X, Eye, EyeOff } from "lucide-react"
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await createStaffUser({
        ...
        name: form.name,
        role: "trainer",
        position: form.specialization || "Trainer",
        department: "Academic",
      })
      resetModal()
      loadTrainers()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowModal(true)}><UserPlus className="h-4 w-4 mr-2" /> Add Trainer</Button>
      </div>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex justify-between items-center mb-5">
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-0.5">Creates a trainer account they can log in with</p>
                </div>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                    onBlur={() => handleBlur("email")} placeholder="trainer@example.com" required className={inputCls("email")} />
                ...
                <div className="space-y-1.5">
                  ...
                  <input value={form.specialization} onChange={e => setForm(p => ({ ...p, specialization: e.target.value }))}
                    placeholder="e.g. AutoCAD, Revit, 3ds Max" className={inputCls("specialization")} />
                </div>
